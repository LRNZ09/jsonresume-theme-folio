...
console.log(
	`[Render Script] Rendering: ${resumeFileRelativePath} using theme '${themeName}'`,
);
...
try {
	...
	console.error(
		`[Render Script] Error: Input resume file not found at ${resumeFilePath}`,
	);
	process.exit(1);
}
try {
...
try {
	...
	console.log(
		`[Render Script] Detected language: ${language} (Theme will use this)`,
	);

	// Output path is now fixed
	console.log(
		`[Render Script] Outputting to fixed path: ${outputFilePathHtml}`,
	);
	...
	console.error(
		`[Render Script] Error processing ${resumeFileRelativePath}:`,
		error.message,
	);
	if (error.stderr) {
