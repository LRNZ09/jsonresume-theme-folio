...
vi.mock('../Summary', () => ({
	Summary: ({ summary }: { summary: string }) => (
		<div data-testid='mock-summary'>{summary}</div>
	),
}));

vi.mock('../ui/ContactInfo', () => ({
	ContactInfo: (props: any) => (
		<div data-testid='mock-contact-info'>{JSON.stringify(props)}</div>
	),
}));

vi.mock('../ui/SocialProfiles', () => ({
	SocialProfiles: (props: any) => (
		<div data-testid='mock-social-profiles'>
...
const getMockBasics = (
	...
	profiles: [
		{ network: 'GitHub', username: 'johndoe', url: 'github.com/johndoe' },
	],
...
describe('Header Component', () => {
	...
	it('renders basic information correctly without image', () => {
		...
		expect(
			screen.getByRole('heading', { level: 1, name: basics.name }),
		).toBeInTheDocument();
		expect(
			screen.getByRole('heading', { level: 2, name: basics.label }),
		).toBeInTheDocument();
		...
		expect(
			screen.getByText(/123 Main St, Beverly Hills, CA, 90210/),
		).toBeInTheDocument();
