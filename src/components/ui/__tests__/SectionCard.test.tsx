...
vi.mock('react-icons/fa', () => ({
	FaExternalLinkAlt: (props: any) => (
		<span data-testid='icon-external-link' {...props} />
	),
	FaMapMarkerAlt: (props: any) => (
		<span data-testid='icon-location' {...props} />
	),
...
describe('SectionCard', () => {
	...
	it('renders title and subtitle correctly without url', async () => {
		...
		expect(
			screen.queryByRole('link', { name: props.subtitle }),
	...
	it('renders action link correctly', async () => {
		...
		renderWithI18n(<SectionCard {...props} />);
		const actionLink = await screen.findByRole('link', {
			name: /view project/i,
		});
		expect(actionLink).toBeInTheDocument();
		expect(actionLink).toHaveAttribute('href', props.actionUrl);
		expect(actionLink).toContainElement(
			screen.getByTestId('icon-external-link'),
		);
	});
